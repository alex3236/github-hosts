...
import { RAW_URL, resolveBaseUrlByHost } from "@/lib/site-config";
...
async function refreshCacheFromDns(): Promise<HostsCache | null> {
  ...
  const cache: HostsCache = {
    updateTime: formatUpdateTime(now),
...
function toHostsData(cache: HostsCache, hostOrUrl?: string): HostsData {
  const { updateTime, entries: cacheEntries } = cache;
  const baseUrl = resolveBaseUrlByHost(hostOrUrl);

  const hostsLines = [
    "#Github Hosts Start",
    `#Update Time: ${updateTime}`,
    "#Project Address: https://github.com/alex3236/github-hosts",
    `#Update URL: ${baseUrl}${RAW_URL}`,
...
export async function parseHosts(hostOrUrl?: string): Promise<HostsData> {
  const cache = cachedHosts;

  if (cache && isCacheFresh(cache)) {
    return toHostsData(cache, hostOrUrl);
  }
  ...
  if (refreshed) {
    return toHostsData(refreshed, hostOrUrl);
  }
...
export async function getHostsContent(hostOrUrl?: string): Promise<string> {
  return (await parseHosts(hostOrUrl)).raw;
}
